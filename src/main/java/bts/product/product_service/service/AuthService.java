...
import bts.product.product_service.model.response.AccessTokenResponse;
...
public class AuthService {
        ...
        try{
            ...
            if (accessTokenResponse.getStatusCode().is2xxSuccessful()){
                return ResponseEntity.ok()
                        .body(new DataResponse("Success", "Login Success", new Date(), 200, accessTokenResponse.getBody(), null));
            } else{
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                        .body(new DataResponse("False", "login failed", new Date(), 500, null, null));
            }
        } catch (Exception e){
    ...
    public ResponseEntity<DataResponse> updateToken(String refreshToken) {
        try{
            ...
            if (accessTokenResponse.getStatusCode().is2xxSuccessful()){
                return ResponseEntity.ok()
                        .body(new DataResponse("Success", "Success Update Token", new Date(), 200, accessTokenResponse.getBody(), null));
            } else{
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                        .body(new DataResponse("False", "Failed Update Token", new Date(), 500, null, null));
            }
        } catch (Exception e){
